...
describe("reachRegStatus", () => {
  ...
  it("Ganaraska lower: closed in the Sept 1–Oct 14 sanctuary, open before and after", () => {
    const gan = { id: "ganaraska", zone: "FMZ 17", species: ["STL", "CHN"] };
    expect(reachRegStatus(gan, new Date(2026, 8, 8)).state).toBe("closed");   // Sep 8
    expect(reachRegStatus(gan, new Date(2026, 7, 20)).state).toBe("open");    // Aug 20
    expect(reachRegStatus(gan, new Date(2026, 9, 20)).state).toBe("open");    // Oct 20
    expect(reachRegStatus(gan, new Date(2026, 0, 15)).state).toBe("closed");  // mid-Jan
    expect(reachSpeciesStates(gan, new Date(2026, 9, 20))[0].window).toMatch(/Aug 31.*Dec 31/);
  });
});
