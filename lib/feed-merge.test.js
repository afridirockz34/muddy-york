...
describe("mergeFeed", () => {
  it("puts user posts (newest first) above the auto-intel items", () => {
    const posts = [
      { id: "p1", createdAt: "2026-08-05T00:00:00Z" },
      { id: "p2", createdAt: "2026-08-07T00:00:00Z" },
    ];
    const derived = [
      { id: "d1", ts: "2026-08-06T00:00:00Z" }, // stamped "now"; must not outrank posts
      { id: "d2", ts: "2026-08-04T00:00:00Z" },
    ];
    expect(mergeFeed(posts, derived).map((x) => x.id)).toEqual(["p2", "p1", "d1", "d2"]);
  });
