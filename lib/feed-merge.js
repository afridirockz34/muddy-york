...
export function mergeFeed(posts = [], derived = []) {
  // One timeline: posts and the ambient auto-intel items interleaved by when they
  // were added, newest first — no group is pinned above another.
  const p = posts.filter((x) => x && !x.deletedAt).map((x) => ({ ...x, kind: "post", _t: ts(x.createdAt) }));
  const d = derived.map((x) => ({ ...x, kind: "derived", _t: ts(x.ts) }));
  return [...p, ...d].sort((a, b) => b._t - a._t);
}
