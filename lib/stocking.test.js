...
describe("parseStocking", () => {
  ...
  it("reads the real Ontario field names + Latitude/Longitude attributes", () => {
    const json = { features: [{ attributes: { Species: "Brook Trout", Stocking_Year: 2023, Developmental_Stage: "Yearling", Latitude: 43.71, Longitude: -80.01 } }] };
    const r = parseStocking(json, loc, 2026);
    expect(r.events[0].species).toBe("Brook Trout");
    expect(r.events[0].yearsAgo).toBe(3);
    expect(r.events[0].distanceKm).toBeLessThan(3);
  });
